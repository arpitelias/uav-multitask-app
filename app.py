...
def download_from_hf(filename,local_path):
    ...
    try:
        with st.spinner(f'Downloading {filename} from Hugging Face (first-time only, ~100MB)...'):
...
def preprocess_image(pil_img):
    ...
    tensor=_normalize(pil_img).unsqueeze(0)
...
def heatmap_to_rgb(heatmap_2d):
    h_norm=(heatmap_2d-heatmap_2d.min())/(heatmap_2d.max()-heatmap_2d.min()+1e-6)
    cmap=plt.get_cmap('jet')
    colored=cmap(h_norm)[:,:,:3]
...
if mode=='Single Model Inference':
        ...
        if source=='Upload':
            ...
            if os.path.exists(samples_dir):
                ...
                if sample_files:
                    ...
                    pil_img=Image.open(os.path.join(samples_dir,selected)).convert('RGB')
                else:
    ...
    with col_main:
        if pil_img is not None:
            ...
            if model is None:
                st.error("Model could not be loaded. Please check your internet connection and try again.")
            else:
                if not loaded:
                    st.warning(f"⚠️ Weights not loaded. Using random initialisaton — predictions will not be meaningful.")

                img_tensor,img_resized=preprocess_image(pil_img)
                ...
                if 'seg_mask' in results:
                    with cols[col_idx]:
                        ...
                        overlay=blend_images(img_resized,seg_rgb,alpha=0.5)
                ...
                if 'det_heatmap' in results:
                    with cols[col_idx]:
                        ...
                        overlay=blend_images(img_resized,heatmap_rgb,alpha=0.4)
                        st.image(overlay,use_column_width=True)
                        col_idx+=1
            ...
            Upload any drone image (or use the provided samples) and select from different model variants to see:
    ...
    st.markdown("""
    - **Frameworks:** PyTorch, torchvision, PIL
